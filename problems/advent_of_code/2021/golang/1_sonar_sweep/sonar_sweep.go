...
func count_depth_increases(values []int) int {
	result := 0
	previous_measure := values[0]
	for i := 1; i < len(values); i++ {
		current_meausre := values[i]
		if current_meausre > previous_measure {
			result += 1
		}
		previous_measure = current_meausre
	}
	return result
}
...
func main() {
	...
	fmt.Println(count_depth_increases(values))
}
